...
from email_utils import send_email, send_registration_otp_email
...
def send_register_otp():
    ...
    try:
        send_registration_otp_email(email, otp, verify_link)
    except Exception as e:
...
def verify_register_otp():
    ...
    data = request.get_json(silent=True) or {}
    email = data.get('email', '').strip().lower()
    otp_input = data.get('otp', '').strip().replace(' ', '').replace('-', '')
    ...
    if not otp_data:
        return {'success': False, 'message': 'No active verification request found for this email. Please click Send OTP.'}, 400

    if otp_input != str(otp_data.get('otp', '')).strip():
        return {'success': False, 'message': 'Invalid OTP code. Please verify the code sent to your email.'}, 400
